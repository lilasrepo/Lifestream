...
public partial class FileSystemSelector<T, TStateStorage>
{
    ...
    private (Vector2, Vector2) DrawFolder(FileSystem<T>.Folder folder)
    {
        ...
        // API12 ImUtf8.TreeNode walk-back has no (nint, label, flags) overload; emulate explicit ID via PushID
        ImGui.PushID((int)folder.Identifier);
        var recurse = ImUtf8.TreeNode(folder.Name.Replace("%", "%%"), flags);
        ImGui.PopID();
    ...
    private bool DrawList(float width)
    {
        ...
        unsafe
        {
            // API12 ImGui.NET uses the underscored constructor symbol
